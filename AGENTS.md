...
- `src/shared/dataVideoFrame.tsx`: shared vertical data-video frame layout for dark data/race videos. It provides stage/background, header, timeline rail, chart frame, chart-top bar, channel tag, footer, `createDataVideoFrameGeometry`, and the current default Shorts race slot through `createDefaultDataShortsFrameGeometry`.
- Default for new data/race Shorts: use the finalized `FootballMarketValues` slot unless the user asks for a different format. Start from `createDefaultDataShortsFrameGeometry()` / `defaultDataShortsTemplate`: centered title block, left-padded year/timeline rail with channel tag at the rail's right end, chart body below it, and source right-aligned near the lower safe area. This slot uses the KBO-capture-style dark race layout with a chart at `top: 498 + SHORTS_PLATFORM_TOP_CLEARANCE`, `height: 940`, visible content inset around `86px` on the left, and `defaultDataShortsTemplate.chartRightPadding` for the chart-only right padding.
- The older centered `76px / 76px` inset is now a legacy market-cap alignment. Keep it for existing compositions that already depend on it; do not choose it for a new video unless matching that older template is explicitly requested.
