...
const defaultText = 'Which country has won the most World Cups?';
const defaultPublicPath = 'projects/world-cup-winners-race/audio/intro-voiceover.mp3';
const generatedModulePath =
  'src/projects/world-cup-winners-race/generated/introVoiceoverAsset.ts';
...
const speakingRate = Number(args['speaking-rate'] ?? process.env.GOOGLE_TTS_SPEAKING_RATE ?? 1.3);
const pitch = Number(args.pitch ?? process.env.GOOGLE_TTS_PITCH ?? -0.5);
const publicPath = args.output ?? defaultPublicPath;
const text = args.text ?? process.env.GOOGLE_TTS_TEXT ?? defaultText;
